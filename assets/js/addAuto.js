 ...
 */
async function registrarAuto(event) {
  try {
    event.preventDefault(); // Evitar que la página se recargue al enviar el formulario

    const formulario = document.querySelector("#formularioAuto");
    // Crear un objeto FormData para enviar los datos del formulario
    const formData = new FormData(formulario);
    console.log(formData)
    // Enviar los datos del formulario al backend usando Axios
    const response = await axios.post("RegistroAuto.php", formData);
    ...
    if (response.status === 200) {
      ...
      setTimeout(() => {
        ...
        var modal = document.getElementById('agregarAuto');
